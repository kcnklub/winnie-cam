//! Splits the WebM byte stream `/audio` sends into an init segment plus
//! whole clusters, client-side. The backend splits the same stream the same
//! way (see `src/audio/webm.rs`) to fan it out; the frontend needs the same
//! boundaries because MSE's `SourceBuffer.appendBuffer()` must be handed the
//! init segment first and whole clusters after.
//!
...
//!
//! Kept wasm-independent so it unit tests natively.
//!
//! ```text
//! [EBML Header][Segment header ..][Cluster][Cluster][Cluster]...
//!  \_______________________________/       \_____/
//!            Chunk::Init                     Chunk::Cluster
//! ```

/// Hard cap on the internal buffer. If a malformed stream never yields a
/// complete chunk, the buffer is dropped rather than growing without bound.
...
pub enum Chunk {
    /// Everything up to the first cluster. Emitted exactly once per stream.
    Init(Vec<u8>),
    /// One complete cluster. Meaningless without the [`Chunk::Init`].
    Cluster(Vec<u8>),
}
...
impl WebmChunker {
    ...
    /// Feed in newly-read bytes; get back every complete chunk in order.
    /// Bytes that don't yet complete a chunk are retained for the next call.
    ...
    fn take_init(&mut self) -> Option<Chunk> {
        let mut pos = 0;

        loop {
            let header = match read_header(&self.buf[pos..]) {
                Parse::Ok(header) => header,
                // A malformed header this early means the server isn't
                // sending what we think it is; there is nothing to resync to.
                Parse::NeedMoreData | Parse::Malformed => return None,
            };

            if header.id == ID_CLUSTER {
                let init: Vec<u8> = self.buf.drain(..pos).collect();
                self.state = State::Clusters;
                return Some(Chunk::Init(init));
            }

            // Descend into the segment rather than skipping it: clusters
            // live inside it, and on a live stream its size is "unknown".
    ...
    fn take_cluster(&mut self) -> Option<Chunk> {
        let header = match read_header(&self.buf) {
            Parse::Ok(header) => header,
            Parse::NeedMoreData => return None,
            Parse::Malformed => return self.resync(),
        };

        // The backend flushes each cluster before writing it, so sizes are
        // known. Unknown-size clusters are legal EBML though, and then the
        // only end marker is the next cluster starting.
...
/// unknown-size and resync cases, where there is no length to trust.
...
mod tests {
    ...
    fn splits_init_from_clusters() {
        ...
        assert_eq!(
            ...
            vec![
                Chunk::Init(init),
                Chunk::Cluster(element(ID_CLUSTER, b"one")),
                Chunk::Cluster(element(ID_CLUSTER, b"two")),
            ]
        );
    }
    ...
    fn cluster_id_inside_block_data_is_not_a_boundary() {
        ...
        assert_eq!(chunks[1], Chunk::Cluster(element(ID_CLUSTER, &payload)));
    }
    ...
    fn resyncs_after_a_corrupt_element() {
        ...
        assert_eq!(chunker.push(&good), vec![Chunk::Cluster(good)]);
    }

    #[test]
    fn skips_trailing_non_cluster_elements() {
        const ID_TAGS: u32 = 0x1254_C367;
        let (init, mut all) = stream(&[b"one"]);
        all.extend_from_slice(&element(ID_TAGS, b"tags"));
        all.extend_from_slice(&element(ID_CLUSTER, b"two"));
